...
class ConversionModel(BaseModel):
    ...
    def train(self, X: pd.DataFrame, y: pd.Series):
        ...
        # Handle degenerate case: only a single class in y
        unique_classes = np.unique(y)
        if unique_classes.size < 2:
            # Fall back to a dummy classifier that always predicts that class
    ...
    def predict_proba(self, X: pd.DataFrame):
        ...
        # Map to probability of class "1" robustly
        classes = getattr(self.model, "classes_", None)
        if classes is None:
            # Fallback: no class info, return zeros
            return np.zeros(len(X))

        # If class "1" exists, use its column; otherwise probability of 1 is 0

...
async def upload_file(file: UploadFile = File(...)):
    ...
    if not file_ext:
        raise HTTPException(
            status_code=400,
            detail=f"File must have an extension. Allowed extensions: {', '.join(settings.allowed_extensions)}"
        )
    ...
    if len(contents) == 0:
        raise HTTPException(
            status_code=400,
            detail="File is empty. Please upload a non-empty file."
        )
    ...
    try:
        ...
        for idx, row in results_df.iterrows():
            ...
            results.append(ProductResult(
                ...
                cluster_id=int(row.get("cluster_id")) if pd.notna(row.get("cluster_id")) else None,
